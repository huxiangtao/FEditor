...
export default function updateTxtPos(textID, attachedEleID) {
  ...
  let textPosX =
    Math.round((bbox.left - CANVAS_LEFT_MARGIN + bbox.width / 2) * 100) / 100;
  let textPosY =
    Math.round((bbox.top - CANVAS_TOP_MARGIN + bbox.height / 2) * 100) / 100;
  matrix = `matrix(1 0 0 1 ${textPosX - TEXT_ELE_WIDTH / 2} ${textPosY -
    TEXT_ELE_HEIGHT / 2})`;
