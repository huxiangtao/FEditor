  ...
  let placeHolder = (
    <g id="place-holder" className="place-holder">
      <polygon
        ...
        strokeDasharray="3 3"
        strokeWidth={1}
        points={placeHolderPoints.map(p => p.join(",")).join(" ")}
        pointerEvents="none"
      />
    </g>
  );
  ...
  if (isCurve) {
    ...
    handlers = ( // only 4 handlers, aka: start/controlPointStart/controlPointEnd/end point
      <g>
        <g id="handlers">
          <line
            className="control-point-tangent"
            x1={CP[0][0]}
            y1={CP[0][1]}
            x2={CP[1][0]}
            y2={CP[1][1]}
          ...
          {CP.map((p, idx) => (
            <image
              ...
              className="svg-shape control-point-handler handler"
              id={"control-point-" + idx}
              style={{ cursor: "nw-resize" }}
              x={p[0] - CONTROL_POINT_HANDLER_WIDTH / 2}
              y={p[1] - CONTROL_POINT_HANDLER_WIDTH / 2}
              width={CONTROL_POINT_HANDLER_WIDTH}
              height={CONTROL_POINT_HANDLER_WIDTH}
              xmlnsXlink="http://www.w3.org/1999/xlink"
              xlinkHref={SCALE_HANDLER_DATA}
    ...
    // each turning has a dot, no rotate/scale/draw-line handlers
    handlers = (
      <g id="handlers">
        {staticData.handlersPos.map((p, idx) => {
          return (
            <image
              key={idx}
              onMouseDown={handlerDown}
              className="svg-shape polyline-handler handler"
              x={p[0] - SCALE_HANDLER_WIDTH / 2}
              y={p[1] - SCALE_HANDLER_WIDTH / 2}
              width={SCALE_HANDLER_WIDTH}
              height={SCALE_HANDLER_WIDTH}
              xmlnsXlink="http://www.w3.org/1999/xlink"
              xlinkHref={SCALE_HANDLER_DATA}
              preserveAspectRatio="none"
            />
    ...
    // regular shape has 4 scale handlers, 1 rotate handlers, 4 draw-line handlers
    handlers = (
      <g>
        <g id="handlers">
          {staticData.handlersPos.map((p, idx) => {
            if (idx !== 4) {
              return (
                <image
                  key={idx}
                  onMouseDown={handlerDown}
                  className="svg-shape scale-handler handler"
                  id={"scale-handler-" + idx}
                  x={p[0] - SCALE_HANDLER_WIDTH / 2}
                  y={p[1] - SCALE_HANDLER_WIDTH / 2}
                  style={{ cursor: "nw-resize" }}
                  width={SCALE_HANDLER_WIDTH}
            ...
            return (
              <image
                key={idx}
                onMouseDown={handlerDown}
                className="svg-shape rotate-handler handler"
                id={"rotate-handler"}
                style={{ cursor: "crosshair" }}
                x={p[0] - ROTATE_HANDLER_WIDTH / 2}
                y={p[1] - ROTATE_HANDLER_HEIGHT / 2}
                width={ROTATE_HANDLER_WIDTH}
                height={ROTATE_HANDLER_HEIGHT}
                xmlnsXlink="http://www.w3.org/1999/xlink"
                xlinkHref={ROTATE_HANDLER_DATA}
                preserveAspectRatio="none"
              />
            );
          })}
