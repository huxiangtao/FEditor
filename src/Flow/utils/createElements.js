...
let wrap = (shape, o, handlers) => {
  ...
  return (
    <g
      ...
      onContextMenu={handlers.onCtxMenu}
    >
      {shape}
    </g>
  );
          ...
          case "path":
            return (
              <path
                ...
                key={idx}
                {...com.get("attributes").toJS()}
              />
            ); // todo: toJS() is strongly not recommended, try to figure out the common props, and list them one by one
          case "circle":
            return (
              <circle
                className="svg-shape shape"
...
export default function createElements(state, staticData, handlers) {
  return state.shapeList.map(o => {
    ...
    switch (o.get("type")) {
      ...
      case "polyline":
        ...
        if (state.selectedElementID === objID) {
          handlerPoints = staticData.selected.element
            ...
            .getAttribute("points")
            .split(/\s+/)
            .map(p => p.split(",").map(num => parseInt(num)));
        }
        shape = (
          <g>
            <polyline
            ...
            />
            <polyline
              className="svg-shape shape-proxy polyline-shape"
              fill="none"
              strokeWidth={14}
              stroke="#424242"
              points={o.get("points")}
      ...
      case "path":
        shape = (
          <path {...commonProps} className="svg-shape shape" d={o.get("d")} />
        );
        break;
      ...
      case "text":
        ...
        if (attachedTo) {
          staticData.attached[attachedTo].text = objID;
        }
        shape = (
          <g
            key={objID}
            id={"attached-text-" + objID}
            className="svg-shape shape text-container"
            pointerEvents="none"
          >
            <foreignObject x={0} y={0} width={80} height={40}>
              <div
                xmlns="http://www.w3.org/1999/xhtml"
                data-attachedto={o.get("attachedTo")}
                className="attached-text content-editable"
